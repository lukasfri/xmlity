...
use crate::XmlnsDeclaration;

/// Errors that can occur when using this crate.
#[derive(Debug, thiserror::Error)]
pub enum Error {
    /// Error from the `xml` crate.
...
fn serializer_to_string<T>(serializer: EventWriter<Vec<u8>>, value: &T) -> Result<String, Error>
where
    T: Serialize,
{
...
where
    T: Serialize,
{
    serializer_to_string(EventWriter::new(Vec::new()), value)
}
    ...
    T: Serialize,
{
    serializer_to_string(
        EventWriter::new_with_config(
            Vec::new(),
            EmitterConfig {
                indent_string: Cow::Owned((0..indentation).map(|_| ' ').collect::<String>()),
                ..Default::default()
            },
        ),
        value,
...
impl<W: Write> From<EventWriter<W>> for Serializer<W> {
...
impl<W: Write> From<W> for Serializer<W> {
    fn from(writer: W) -> Self {
        Self::new(EventWriter::new(writer))
    }
}
...
impl<'s, W: Write> SerializeElement<'s, W> {
    fn finish_start(self) -> (QName<'static>, &'s mut Serializer<W>) {
        ...
        serializer.buffered_bytes_start.attributes.clear();
        ...
        serializer.buffered_bytes_start.name = todo!();
    ...
    fn end_empty(serializer: &mut Serializer<W>) -> Result<(), Error> {
        ...
        let start = &serializer.buffered_bytes_start;

        let name = start.name.borrow();
        let attributes: Vec<Attribute<'_>> = start.attributes.iter().map(|a| a.borrow()).collect();
        let namespace = todo!();
        ...
        serializer
            .writer
            .write(XmlEvent::EndElement { name: Some(name) })
            .map_err(Error::XmlRs)?;
    ...
    fn end(self) -> Result<Self::Ok, Self::Error> {
        self.serializer
            ...
            .write(XmlEvent::EndElement {
                name: Some(self.end_name.borrow()),
            })
            .map_err(Error::XmlRs)?;
...
impl<W: Write> Serializer<W> {
    ...
    fn push_attr(&mut self, qname: QName<'_>, value: &str) {
        self.buffered_bytes_start.attributes.push(OwnedAttribute {
            name: todo!(),
...
impl<'s, W: Write> xmlity::Serializer for &'s mut Serializer<W> {
    ...
    fn serialize_cdata<S: AsRef<str>>(self, text: S) -> Result<Self::Ok, Self::Error> {
        self.try_start()?;
        self.writer
            .write(XmlEvent::CData(text.as_ref()))
            .map_err(Error::XmlRs)
    }
    ...
    fn serialize_text<S: AsRef<str>>(self, text: S) -> Result<Self::Ok, Self::Error> {
        self.try_start()?;
        self.writer
            .write(XmlEvent::Characters(text.as_ref()))
            .map_err(Error::XmlRs)
    }
    ...
    fn serialize_decl<S: AsRef<str>>(
        ...
        self.writer
            .write(XmlEvent::StartDocument {
                version: XmlVersion::Version11, //TODO
                encoding: encoding.as_ref().map(|s| s.as_ref()),
                standalone: standalone
                    .as_ref()
                    .map(|s| s.as_ref().parse())
                    .transpose()
                    .map_err(|e| Error::Custom(format!("Invalid standalone: {e}")))?,
            })
            .map_err(Error::XmlRs)
    }
    ...
    fn serialize_pi<S: AsRef<[u8]>>(self, text: S) -> Result<Self::Ok, Self::Error> {
        self.try_start()?;
        self.writer
            .write(XmlEvent::ProcessingInstruction {
                name: todo!(),
                data: todo!(),
            })
            .map_err(Error::XmlRs)
    ...
    fn serialize_comment<S: AsRef<[u8]>>(self, text: S) -> Result<Self::Ok, Self::Error> {
        self.try_start()?;
        self.writer
            .write(XmlEvent::Comment(str::from_utf8(text.as_ref()).unwrap()))
            .map_err(Error::XmlRs)
    }
    ...
    fn serialize_doctype<S: AsRef<[u8]>>(self, text: S) -> Result<Self::Ok, Self::Error> {
        self.try_start()?;
        todo!()
        // self.writer.write(todo!()).map_err(Error::XmlRs)
    }
