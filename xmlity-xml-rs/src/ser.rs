...
use crate::{declaration_into_attribute, Attribute, XmlnsDeclaration};
...
impl<'s, W: Write> ser::SerializeElement for SerializeElement<'s, W> {
    ...
    fn end(self) -> Result<Self::Ok, Self::Error> {
        ...
        serializer
            .writer
            .write(XmlEvent::StartElement(bytes_start.as_quick_xml()))?;
...
impl<W: Write> ser::SerializeSeq for ChildrenSerializeSeq<'_, W> {
    ...
    fn end(self) -> Result<Self::Ok, Self::Error> {
        ...
        if let Some(bytes_start) = self.bytes_start {
            self.serializer
                .writer
                .write(XmlEvent::Empty(bytes_start.as_quick_xml()))
                .map_err(Error::Io)?;
        } else {
...
impl<'s, W: Write> xmlity::Serializer for &'s mut Serializer<W> {
    ...
    fn serialize_cdata<S: AsRef<str>>(self, text: S) -> Result<Self::Ok, Self::Error> {
        self.writer
            .write(XmlEvent::CData(BytesCData::new(text.as_ref())))
            .map_err(Error::Io)
    }

    fn serialize_text<S: AsRef<str>>(self, text: S) -> Result<Self::Ok, Self::Error> {
        self.writer
            .write(XmlEvent::Text(BytesText::from_escaped(text.as_ref())))
            .map_err(Error::Io)
    }
    ...
    fn serialize_decl<S: AsRef<str>>(
        ...
        self.writer
            .write(XmlEvent::Decl(BytesDecl::new(
                version.as_ref(),
    ...
    fn serialize_pi<S: AsRef<[u8]>>(self, text: S) -> Result<Self::Ok, Self::Error> {
        self.writer
            .write(XmlEvent::ProcessingInstruction(BytesPI::new(
...
impl<W: Write> SerializerWithPossibleBytesStart<'_, '_, W> {
    pub fn try_start(&mut self) -> Result<(), Error> {
        if let Some(bytes_start) = self.possible_bytes_start.take().and_then(Option::take) {
            self.serializer
                .writer
                .write(XmlEvent::Start(bytes_start.as_quick_xml()))
                .map_err(Error::Io)?;
        }
        Ok(())
    }
